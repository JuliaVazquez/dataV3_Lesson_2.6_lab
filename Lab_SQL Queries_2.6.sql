...
GROUP BY release_year;
	-- there is only 1 release_year


-- 5. Using the film table, find out for each rating how many films were there
SELECT release_year, rating, COUNT(title) FROM sakila.film
GROUP BY rating;



-- 6. What is the mean length of the film for each rating type. Round off the average lengths to two decimal places
SELECT rating, COUNT(title), ROUND(AVG(length),2) AS avg_length FROM sakila.film
GROUP BY rating;
...
SELECT title, length, RANK()OVER(ORDER BY length DESC) AS my_rank FROM sakila.film
